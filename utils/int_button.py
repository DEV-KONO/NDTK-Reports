...
def incr_btn_qty(e, text):
    text.value = str(int(text.value) + 1)
    e.page.update()

def decr_btn_qty(e, text):
    text.value = str(int(text.value) - 1)
    e.page.update()
...
def Stepper_qty():
    text =  ft.TextField(
        ...
        width=150,
        height=60,
        prefix=ft.IconButton(ft.icons.ARROW_UPWARD, icon_color=ft.colors.BLACK, on_click=lambda e: incr_btn_qty(e, text)),
        suffix=ft.IconButton(ft.icons.ARROW_DOWNWARD, icon_color=ft.colors.BLACK, on_click=lambda e: decr_btn_qty(e, text)),
        border_radius=10,
        show_cursor=False,
        color="black",
        input_filter=ft.NumbersOnlyInputFilter()
